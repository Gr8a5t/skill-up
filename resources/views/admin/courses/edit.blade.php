...
@push('styles')
    <style>
        ...
        select.form-control option { background: #0a0a0c; }
        ...
        @media (max-width: 640px) { .form-grid { grid-template-columns: 1fr; } .lesson-item { grid-template-columns: 36px 1fr 36px; } }
    </style>
@endpush

@section('content')
    <div class="page-header">
        <h1 class="page-title">Edit: {{ $course->title }}</h1>
        <p class="page-sub">Update course details, lessons, and resources.</p>
    </div>

    <form action="{{ route('admin.courses.update', $course) }}" method="POST" class="form-container">
        @csrf @method('PUT')
                    ...
                    <div class="input-group">
                        ...
                        <select name="category" class="form-control">
                            @foreach(['Frontend','Backend','Mobile Dev','AI & Data','Design','DevOps'] as $cat)
                                <option {{ old('category', $course->category) == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="input-group">
                        ...
                        <select name="level" class="form-control">
                            @foreach(['Beginner','Medium','Advance'] as $lvl)
                                <option {{ old('level', $course->level) == $lvl ? 'selected' : '' }}>{{ $lvl }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="input-group">
                        <label>Icon Name (IonIcon)</label>
                        <input type="text" name="icon" class="form-control" value="{{ old('icon', $course->icon) }}" placeholder="e.g. logo-javascript">
                        <p class="field-hint">Browse at <a href="https://ionic.io/ionicons" target="_blank" style="color:var(--brand-primary)">ionicons.io</a></p>
                    </div>
                    <div class="input-group">
                        <label>Card Background Color</label>
                        <input type="color" name="color" class="form-control" value="{{ old('color', $course->color) }}" style="height:48px; padding:4px 8px; cursor:pointer;">
                    </div>
                    <div class="input-group full-width">
                        <label>YouTube Playlist ID</label>
                        <input type="text" name="playlist_id" class="form-control" value="{{ old('playlist_id', $course->playlist_id) }}" placeholder="e.g. PL4cUxeGkcC9...">
                    </div>
                </div>
            </div>
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    <div class="input-group full-width">
                        ...
                        <textarea name="recap" class="form-control">{{ old('recap', $course->recap) }}</textarea>
                    </div>
                    <div class="input-group full-width">
                        <label>Key Concepts <span style="text-transform:none; opacity:0.6; font-size:0.9em;">– one per line</span></label>
                        <textarea name="key_concepts" class="form-control" style="min-height:90px;">{{ old('key_concepts', is_array($course->key_concepts) ? implode("\n", $course->key_concepts) : $course->key_concepts) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="form-section">
                ...
                <div class="lesson-list" id="lessonList">
                    ...
                    <div class="lesson-item" data-index="{{ $i }}">
                        ...
                        <input type="text" name="lessons[{{ $i }}][title]" class="form-control" value="{{ $lesson->title }}" placeholder="Lesson Title" required>
                        <input type="text" name="lessons[{{ $i }}][video_id]" class="form-control" value="{{ $lesson->video_id }}" placeholder="YouTube Video ID" required>
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    <div class="input-group">
                        ...
                        <input type="url" name="source_files_url" class="form-control" value="{{ old('source_files_url', $course->source_files_url) }}" placeholder="https://github.com/...">
                    </div>
                    <div class="input-group">
                        <label>Cheat Sheet / Extra URL <span style="text-transform:none; opacity:0.6; font-size:0.85em;">– optional</span></label>
                        <input type="url" name="cheatsheet_url" class="form-control" value="{{ old('cheatsheet_url', $course->cheatsheet_url) }}" placeholder="https://docs.example.com/...">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="action-card">
            ...
            <button type="submit" class="btn-publish"><ion-icon name="checkmark-circle-outline"></ion-icon> Update Course</button>
            <a href="{{ route('admin.courses') }}" class="btn-back">← Back to Courses</a>
        </div>
    </form>
@endsection
...
@push('scripts')
<script>
    let lessonCount = {{ $course->lessons->count() ?: 1 }};
    function addLesson() {
